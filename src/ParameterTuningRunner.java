...
/**
 * Stage 2 Parameter Tuning: Full Factorial on coolingRate x betaMax.
 *
 * Sequential experimentation (Montgomery, 2017, Ch. 12):
 *   Stage 1 OFAT screening identified:
 *     - maxIterations:  monotonic improvement -> fix at 10000 (resource param)
 *     - coolingRate:    spread=1.46, best=0.9990 -> TUNE (Factor A)
 *     - betaMax:        spread=0.73, best=10     -> TUNE (Factor B)
 *     - segmentLength:  spread=0.44  -> fix at 100
 *     - reactionFactor: spread=0.04  -> fix at 0.8
 *
 * Design: 3x3 full factorial with 3 replications (seeds)
...
public class ParameterTuningRunner {
    ...
    static class Config {
        ...
        // Coded DOE levels (-1, 0, +1) for ANOVA
        int codedA;  // coolingRate
        int codedB;  // betaMax

        Config(String name, int maxIter, int segLen, double cooling,
               int bMin, int bMax, double reaction, int cA, int cB) {
            this.name = name;
     ...
     *
     *   Factor A (coolingRate):  0.9990 (-1),  0.9995 (0),  0.9999 (+1)
     *   Factor B (betaMax):      4 (-1),       6 (0),       10 (+1)
     *
     * Fixed (from Stage 1):
     *   maxIterations=10000, segmentLength=100, betaMin=2, reactionFactor=0.8
     */
    private static List<Config> buildConfigurations() {
        List<Config> configs = new ArrayList<>();

        double[] COOL    = {0.9990, 0.9995, 0.9999};
        int[]    BMAX    = {4,      6,      10};
        int[]    CODED   = {-1, 0, +1};

        // Fixed parameters
        int    ITER  = 10000;   // best from Stage 1 (resource parameter)
        int    SEG   = 100;     // not significant (spread=0.44)
        int    BMIN  = 2;       // fixed
        double REACT = 0.8;     // not significant (spread=0.04)

        for (int a = 0; a < 3; a++) {
            for (int b = 0; b < 3; b++) {
                String name = String.format("c%.4f_b%d", COOL[a], BMAX[b]);
                configs.add(new Config(name, ITER, SEG, COOL[a],
                        BMIN, BMAX[b], REACT, CODED[a], CODED[b]));
            }
        }

        return configs;  // 9 configurations
    }
    ...
    public static void run(String instanceFolder, String outputPath,
                           double syncWindow, long[] seeds) throws IOException {
        ...
        int totalRuns = nInstances * nConfigs * nSeeds;

        System.out.println("╔══════════════════════════════════════════════════════════╗");
        System.out.println("║  CTOP-T-Sync: Stage 2 Factorial Experiment              ║");
        System.out.println("╠══════════════════════════════════════════════════════════╣");
        System.out.printf( "║  Design:          3x3 Full Factorial (AxB) x %d seeds%n", nSeeds);
        System.out.printf( "║  Factor A:        coolingRate   {0.9990, 0.9995, 0.9999}%n");
        System.out.printf( "║  Factor B:        betaMax       {4, 6, 10}%n");
        System.out.printf( "║  Fixed:           I=10000, seg=100, lam=0.8%n");
        System.out.printf( "║  Seeds:           %s%n", Arrays.toString(seeds));
        ...
        // Print design matrix
        System.out.println("  Design Matrix (9 configs x 3 seeds = 27 treatments):");
        System.out.println("  -------------------------------------------------------");
        System.out.printf( "  %-3s  %-18s  %8s  %4s  %3s  %3s%n",
                "#", "Name", "Cool", "bMax", "A", "B");
        System.out.println("  -------------------------------------------------------");
        for (int i = 0; i < configs.size(); i++) {
            Config c = configs.get(i);
            System.out.printf("  %-3d  %-18s  %.4f  %4d  %+2d  %+2d%n",
                    i, c.name, c.coolingRate, c.betaMax, c.codedA, c.codedB);
        }
        System.out.println();
        ...
        csv.println(String.join(",", new String[]{
                "Config_ID", "Config_Name", "Seed",
                "Cooling_Rate", "Beta_Max",
                "A_Cool", "B_BMax",
        ...
        // ── Run all (config, seed, instance) triples ──
        int runCount = 0;
        ...
        for (int ci = 0; ci < nConfigs; ci++) {
            ...
            for (int si = 0; si < nSeeds; si++) {
                ...
                originalOut.printf("  [Config %d/%d, Seed %d/%d] %s  (seed=%d)%n",
                        ci + 1, nConfigs, si + 1, nSeeds, config.name, seed);
                ...
                for (int fi = 0; fi < nInstances; fi++) {
                    ...
                    try {
                        ...
                        // Write CSV row
                        csv.printf("%d,%s,%d,%.4f,%d,%d,%d,%s,%d,%d,%.0f,%.0f,%.1f,%.0f,%d,%d,\"%s\",%d,%s,%d,%.1f%n",
                                ci, config.name, seed,
                                config.coolingRate, config.betaMax,
                                config.codedA, config.codedB,
                                fileName,
                        ...
                        csv.printf("%d,%s,%d,%.4f,%d,%d,%d,%s,,,,,,,,\"\",,FALSE,,%n",
                                ci, config.name, seed,
                                config.coolingRate, config.betaMax,
                                config.codedA, config.codedB,
                                fileName);
                ...
                originalOut.printf("%n    Done in %.1fs%n%n", configTime / 1000.0);
            }
        }
        ...
        originalOut.println();
        originalOut.println("════════════════════════════════════════════════════════════");
        originalOut.printf( "  STAGE 2 FACTORIAL EXPERIMENT COMPLETE%n");
